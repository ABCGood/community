...
$(document).ready(function() {
    $.ajax({
        url : "https://www.easy-mock.com/mock/5aea779a9d0dc53dc5af4c22/jiannanxc/name",//请求地址
        dataType : "json",//数据格式
        type : "get",//请求方式
        async : false,//是否异步请求
        success : function(data) {   //如何发送成功
            var dataContent = data.data.name;
            var html = "<div class='swiper-slide selected'>"+dataContent[0]+"</div>";
            for (var i = 1; i < dataContent.length; i++) {    //遍历data数组
                var ls = dataContent[i];
                html += "<div class='swiper-slide'>"+ls+"</div>";
            }
            $("#daohang").html(html);
        }
    })
...
window.onload = function () {
    ...
    // $.ajax({
    //     url : "https://www.easy-mock.com/mock/5aea779a9d0dc53dc5af4c22/jiannanxc/JianNanxc?page=2",//请求地址
    //     dataType : "json",//数据格式
    //     type : "get",//请求方式
    //     async : false,//是否异步请求
    //     success : function(data) {   //如何发送成功
    ...
    // })
    // // console.log(result)
    // laytpl($('#index_content').html()).render(result.data, function (html) {
    //     // console.log(html)
    //     if (result.data){
    //         $('#textIndexContent').append(html)
    //     } else {
    //         $('#textIndexContent').html(html)
    //     }
    // });
    // laytpl($('#index_contentGG').html()).render(result.data, function (html) {
    //     //得到的模板渲染到html
    //     if (result.data){
    //         $('#textGGIndexContent').append(html)
    //     } else {
    //         $('#textGGIndexContent').html(html)
    //     }
    // });
    ...
    layui.use('flow', function () {
        ...
        flow.load({
            elem: '#textIndexContent' //指定列表容器
            , done: function (page, next) { //到达临界点（默认滚动触发），触发下一页
                // console.log(page)
                var lis = [];
                //以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
                $.get('https://www.easy-mock.com/mock/5aea779a9d0dc53dc5af4c22/jiannanxc/JianNanxc?page='+page, function (res) {
                    //假设你的列表返回在data集合中
                    layui.each(res.data, function (index, item) {
                        // console.log("item------")
                        // console.log(item)
                        // console.log(item[0].con)
                        laytpl($('#index_content').html()).render(item[0].con, function (html) {
                            // console.log(html)
                            if (item[0].con){
                            ...
                            //得到的模板渲染到html
                            // document.getElementById('textIndexContent').innerHTML = html;
                        });
                    });
    ...
    layui.use('flow', function () {
        ...
        var flow = layui.flow;
        flow.load({
            elem: '#textGGIndexContent' //指定列表容器
            , done: function (page, next) { //到达临界点（默认滚动触发），触发下一页
                console.log(page)
                var lis = [];
                //以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
                $.get('https://www.easy-mock.com/mock/5aea779a9d0dc53dc5af4c22/jiannanxc/JianNanxc?page='+page, function (res) {
                    ...
                    layui.each(res.data, function (index, item) {
                        ...
                        // console.log(item)
                        // console.log(item[0].con)
                        laytpl($('#index_contentGG').html()).render(item[0].con, function (html) {
                            //得到的模板渲染到html
                            if (item[0].con){
                                $('#textGGContent').append(html)
                            } else {
                                $('#textGGContent').html(html)
                            }
                        });
                    });
